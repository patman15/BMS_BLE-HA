...
8. **Runtime** (experimental):
   ...
   - When charging, the base class calculates charge_time instead
...
- `sensor.bms_charge_time` - Estimated charge time when charging (seconds)
- `binary_sensor.bms_battery_charging` - Charging status

### Calculated Values
- **cycle_capacity**: Stored energy (Wh) = voltage × cycle_charge
- **charge_time**: Calculated by base class when charging
