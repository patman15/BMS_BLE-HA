...
class BMS(BaseBMS):
    ...
    async def _async_update(self) -> BMSsample:
        ...
        for i, (cmd, cmd_name) in enumerate(zip(INIT_COMMANDS, INIT_COMMAND_NAMES)):
            LOGGER.debug(
        ...
        LOGGER.debug(
            f"Packet stats - Total: {self._packet_stats['total']}, "
            f"Init: {self._packet_stats['init_responses']}, "
            f"Data: {self._packet_stats['data_packets']}, "
            f"Invalid: {self._packet_stats['invalid']}"
        )
        if self._packet_stats['unknown_types']:
            LOGGER.debug(f"Unknown packet types: {self._packet_stats['unknown_types']}")
    ...
    def _parse_realtime_packet(self, packet: bytes) -> bool:
        ...
        try:
            ...
            if temperature <= 25.5:  # Always true for unsigned byte, but kept for clarity
                ...
                LOGGER.warning(f"Temperature out of range: {temperature:.1f}°C (raw: {temp_raw})")
            ...
            LOGGER.error("Failed to parse packet: %s", e, exc_info=True)
    ...
    def _process_data(self) -> Optional[BMSsample]:
        ...
        LOGGER.debug(f"Created BMSsample: {sample}")
        return sample
