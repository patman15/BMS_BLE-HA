...
class OGTBms:
    ...
    UUID_RX = normalize_uuid_str("FFF4") # '0000fff4-0000-1000-8000-00805f9b34fb'
    ...
    reconnect = False) -> None:
        ...
        self._key = sum(self._CRYPT_NAME[int(c,16)] for c in (f'{int(self._ble_device.name[10:]):0>4X}')) + (5 if (self._ble_device.name[9] == 'A') else 8)      
        self.logger.info(f"Offgridtec LiFePo4 Smart Pro type: {self._type}, ID: {self._ble_device.name[10:]}, key: 0x{self._key:0>2X}")
        self._values = {} # dictionary of queried values

        if self._type == 'A':
            self._OGT_REGISTERS = {
                2: dict(name = "battery_level", len = 1, func = lambda x: x), # SOC (State of Charge)
                4: dict(name = "cycle_capacity", len = 3, func = lambda x: float(x)/1000),
                8: dict(name = "voltage", len = 2, func = lambda x: float(x)/1000),
                12: dict(name = "temperature", len = 2, func = lambda x: round(float(x)*0.1-273.15, 1)), # MOS temperature
            ...
            self._OGT_REGISTERS = {
                8: dict(name = "temperature", len = 2, func = lambda x: round(float(x)*0.1-273.15, 1)), # MOS temperature
                9: dict(name = "voltage", len = 2, func = lambda x: float(x)/1000),
                10: dict(name = "current", len = 3, func = lambda x: float(x)/1000),
                13: dict(name = "battery_level", len = 1, func = lambda x: x), # SOC (State of Charge)
    ...
    async def update(self) -> dict:
        ...
        try:
            ...
            self.logger.debug(f"failed to connect: {str(e)} ({type(e).__name__})")
            raise IOError
        ...
        if {"cycle_capacity", "voltage"}.issubset(self._values.keys()):
            ...
            self._values["cycle_capacity"] = round(self._values["cycle_capacity"]*self._values["voltage"], 6)
        else:
            del self._values["cycle_capacity"]
            
        self.logger.debug("data collected: %s", self._values)
        if self._reconnect:            
            await self._disconnect() # disconnect after data update to force reconnect next time (slow!)
        return self._values
    ...
    def _notification_handler(self, sender, data: bytearray) -> None:
        ...
        if valid and sender.uuid == self.UUID_RX:
            ...
            self.logger.debug(f"reg: {register['name']} (#{reg}), raw: {nat_value}, value: {value}")
    ...
    def _ogt_response(self, resp: bytearray) -> tuple:
        ...
        msg = bytearray(((resp[x]^self._key) for x in range (0, len(resp)))).decode(encoding="ascii")
        ...
        signed = len(msg)>12
        value = int.from_bytes(bytes.fromhex(msg[6:10]), byteorder='little', signed=signed) * (int(msg[10:12],16) if signed else 1)
