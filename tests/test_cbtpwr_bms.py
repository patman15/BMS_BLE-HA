...
class MockCBTpwrBleakClient(MockBleakClient):
    ...
    def _response(
        ...
        if cmd in (0x07, 0x08):
            assert False, "only 5 cells available, do not query."
        if cmd == 0x0B:
            return bytearray(
...
class MockInvalidBleakClient(MockCBTpwrBleakClient):
    ...
    def _response(
        ...
        cmd: int = int(bytearray(data)[2])
        if cmd == 0x0B:
            return bytearray(b"invalid_len")  # invalid length
        if cmd == 0x09:
            return bytearray(b"\x12\x34\x00\x00\x00\x56\x78")  # invalid start/end
        if cmd == 0x15:
            return bytearray(
