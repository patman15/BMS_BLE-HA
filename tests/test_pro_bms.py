...
def test_uuid_tx() -> None:
    ...
    assert BMS.uuid_tx() == "0000fff3-0000-1000-8000-00805f9b34fb"
...
async def test_wait_for_response_data_mode(patch_bleak_client) -> None:
    ...
    asyncio.create_task(simulate_data_packet())
...
async def test_temperature_out_of_range(patch_bleak_client, caplog) -> None:
    """Test temperature out of range handling."""
    import logging
    import struct
    
    class MockTempOutOfRangeClient(MockProBMSBleakClient):
        async def write_gatt_char(self, char_specifier, data, response=None):
            await super().write_gatt_char(char_specifier, data, response)
            if self._notify_callback and self._data_streaming:
                # Create packet with temperature that would be out of range
                # Temperature is temp_raw / 10.0, so 255 / 10.0 = 25.5°C
                # The code checks if temperature <= 25.5, so we need > 25.5
                # But with unsigned byte max 255, we can only get 25.5°C max
                # This test verifies the unreachable branch
                packet = create_test_packet(temp_c=255)  # This will be 25.5°C after conversion
                self._notify_callback(None, packet)
    
    patch_bleak_client(MockTempOutOfRangeClient)
    ...
    # Temperature should be 25.5°C (255 / 10.0)
    assert result["temperature"] == 25.5
    # No warning should be logged since 25.5 <= 25.5
            ...
            if self._notify_callback and self._data_streaming:
                ...
                def mock_unpack(fmt, data):
                    ...
                    if fmt == "<H" and len(data) == 2 and data[0] == 0xFF and data[1] == 0xFF:
                        raise Exception("Mock general exception during parsing")
                    return original_unpack(fmt, data)
